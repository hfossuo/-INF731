        ...
        static public List<Article> lectureFacture(string nom_facture_sortant, List<Article> list)
        {
            if(isFileExist(nom_facture_sortant))
            {
                ...
                while (!flux_sortant.EndOfStream)
                {
                    ...
                    if (element.Length != 5)
                    {
                        throw new Exception(String.Format("Erreur dans le fichier {0} format incorrect a la ligne {1}", nom_facture_sortant, ligne));
                        // A ajouter nom du fichier et nom colonne
                    }
